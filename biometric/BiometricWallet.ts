...
export class BiometricWallet {
  ...
  public async enrollBiometricIdentity(
    ...
    try {
      ...
      console.log(`✅ Biometric identity enrolled successfully`);
    ...
    try {
      ...
      if (success) {
        ...
        if (keyPair) {
          ...
          console.log(`✅ Biometric authentication successful (${(confidence * 100).toFixed(1)}%)`);
  ...
  public async updateBiometricTemplate(newReadings: BiometricReading[]): Promise<boolean> {
    try {
      ...
      console.log(`🔄 Updating biometric template for ${this.validatorId}...`);
      ...
      console.log(`✅ Biometric template updated`);
      return true;
