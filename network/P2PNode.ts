  ...
  private async startPeerDiscovery(): Promise<void> {
    ...
    setInterval(async () => {
      ...
      if (peerCount < this.config.minConnections) {
        console.log(`⚠️  Low peer count (${peerCount}/${this.config.minConnections}), discovering more peers...`);
  ...
  public updatePeerReputation(peerId: string, delta: number): void {
    ...
    if (peerInfo) {
      ...
      if (peerInfo.reputation < -50) {
        console.warn(`⚠️  Peer ${peerId.substring(0, 12)}... has low reputation: ${peerInfo.reputation}`);
