...
export class BlockPropagation extends EventEmitter {
  ...
  public async propagateBlock(block: Block, excludePeers: string[] = []): Promise<PropagationResult> {
    ...
    try {
      ...
      console.log(`✅ Block propagation completed: ${block.hash} (${successful}/${targetPeers.length} peers, ${propagationTime}ms)`);
      ...
      const propagationTime = Date.now() - startTime;
      console.error(`❌ Block propagation failed: ${block.hash}`, error);
  ...
  public async validateIncomingBlock(block: Block, source: string): Promise<ValidationResult> {
    ...
    try {
      ...
      if (isValid) {
        console.log(`✅ Block validation passed: ${block.hash} (${validationTime}ms)`);
        this.emit('blockValidated', { block, validationResult, source });
      } else {
        console.warn(`⚠️ Block validation failed: ${block.hash} - ${errors.join(', ')}`);
      ...
      const validationTime = Date.now() - startTime;
      console.error(`❌ Block validation error: ${block.hash}`, error);
    ...
    try {
      ...
      if (parentFound) {
        ...
        if (validation.isValid) {
          ...
          console.log(`✅ Orphan block resolved: ${orphanId}`);
          ...
          console.warn(`⚠️ Orphan block validation failed: ${orphanId}`);
      ...
      console.error(`❌ Error handling orphan block ${orphanId}:`, error);
  ...
  private async handleIncomingBlock(peerId: string, block: Block): Promise<void> {
    console.log(`📦 Received block from ${peerId}: ${block.hash}`);
    ...
    if (validation.isValid) {
      ...
      console.warn(`⚠️ Invalid block received from ${peerId}: ${validation.errors.join(', ')}`);
  ...
  private processOrphanBlocks(): void {
    ...
    for (const orphanId of expiredOrphans) {
      this.orphanBlocks.delete(orphanId);
      this.metrics.orphanBlocks--;
      console.log(`🗑️ Removed expired orphan block: ${orphanId}`);
    }
  }
